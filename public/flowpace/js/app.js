...
async function syncToServer() {
  ...
  try {
    const payload = {
      ...
      device_id: localStorage.getItem('flowpace_device_id') || 'default',
    };
    await api('POST', '/api/flowpace/sync', payload);
...
async function syncFromServer() {
  if (!state.isOnline) return;
  try {
    const data = await api('GET', '/api/flowpace/sync');
    ...
    if (data.completions && data.completions.length) {
      ...
      for (const c of data.completions) await db.saveCompletion(c);
    }
...
async function loadLocalData() {
  state.routines = await db.getRoutines();
  state.completions = await db.getCompletions();
...
async function loadData() {
  ...
  if (state.isOnline) {
    try {
      ...
      const serverData = await api('GET', '/api/flowpace/sync');
      if (serverData.routines && serverData.routines.length) {
        await db.clear('routines');
        for (const r of serverData.routines) await db.saveRoutine(r);
        state.routines = serverData.routines;
      }
      if (serverData.completions && serverData.completions.length) {
        await db.clear('completions');
        for (const c of serverData.completions) await db.saveCompletion(c);
        state.completions = serverData.completions;
      }
      if (serverData.settings) {
        await db.saveSettings(serverData.settings);
        state.settings = { ...state.settings, ...serverData.settings };
        delete state.settings.key;
      }
    } catch (e) {
      // server empty or error, use local
    }
  }
  render();
}
...
function renderList() {
  ...
  document.getElementById('routine-count').textContent = state.routines.length;
  if (!state.routines.length) {
    ...
    document.getElementById('list-actions').innerHTML = `<button class="btn btn-glass w-full" onclick="openEditor()">➕ Create New Routine</button>`;
    return;
  }
  ...
  el.querySelectorAll('.routine-row').forEach(row => {
    ...
    row.addEventListener('drop', async e => {
      ...
      if (dragSrc !== row) {
        ...
        await db.clear('routines');
        for (const r of state.routines) await db.saveRoutine(r);
        await syncToServer();
        renderList();
      }
    });
  });
  document.getElementById('list-actions').innerHTML = `
...
async function duplicateRoutine(id) {
  const source = state.routines.find(x=>x.id===id);
  if (!source) return;
  const dup = JSON.parse(JSON.stringify(source));
  dup.id = uuid();
  dup.name = dup.name + ' (Copy)';
  for (const item of dup.steps) {
    item.id = uuid();
    if (item.steps) {
      for (const s of item.steps) s.id = uuid();
    }
  }
  await db.saveRoutine(dup);
  state.routines.push(dup);
  await syncToServer();
  renderList();
...
async function saveRoutine() {
  ...
  const id = state.editingRoutine ? state.editingRoutine.id : uuid();
  const routine = { id, name, steps: editorItems };
...
function startTimer(id) {
  ...
  state.currentStepIndex = 0; state.totalElapsed = 0;
  state.totalDuration = state.flattenedSteps.reduce((a,s)=>a+s.duration,0);
  state.lastAnnouncedSecond = null; state.didAnnounceComplete = false;
  updateTimerDisplay();
  showView('timer');
  resumeTimer();
}
...
function timerTick() {
  ...
  state.timeRemaining -= 0.1; state.totalElapsed += 0.1;
...
function moveToNextStep() {
  ...
  if (state.currentStepIndex >= state.flattenedSteps.length) { completeRoutine(); return; }
  const step = state.flattenedSteps[state.currentStepIndex];
  state.timeRemaining = step.duration; state.currentStepDuration = step.duration;
  state.lastAnnouncedSecond = null; state.didAnnounceComplete = false;
  playBeep(900,0.15); speak(`Next: ${step.name}`,0.5,1);
  updateTimerDisplay();
}
...
function resetTimer() {
  pauseTimer(); state.timerState = 'idle'; state.currentStepIndex = 0; state.totalElapsed = 0;
  state.lastAnnouncedSecond = null; state.didAnnounceComplete = false;
...
function updatePlayBtn() {
  document.getElementById('btn-play').textContent = state.timerState === 'running' ? '⏸' : '▶';
}
function exitTimer() {
  pauseTimer(); stopAudio(); resetTimer();
  setBg(state.settings.background_color);
  goBack();
}
...
async function setBgColor(color) {
  ...
  setBg(color); renderSettings(); await syncToServer();
}
...
async function setSoundPack(id) {
  ...
  await db.saveSettings(state.settings);
  renderSettings(); showToast('Sound pack updated'); await syncToServer();
}
function previewPack(id) { playBeep(800+Math.random()*400,0.2); }
...
async function renderAnalytics() {
  ...
  const filtered = comps.filter(c => new Date(c.completed_at) >= start);
  const total_sessions = filtered.length;
  const total_time = filtered.reduce((a,c)=>a+(c.total_duration||0),0);
  const avg_duration = total_sessions ? total_time/total_sessions : 0;

  // streak
  const sorted = [...comps].sort((a,b)=>new Date(b.completed_at)-new Date(a.completed_at));
  let streak = 0; let check = new Date(); check.setHours(0,0,0,0); const seen = new Set();
  for (const c of sorted) {
    const d = new Date(c.completed_at); d.setHours(0,0,0,0);
    const diff = (check - d) / (86400000);
    if (diff === 0 || (diff === 1 && !seen.has(d.getTime()))) {
      if (!seen.has(d.getTime())) { streak++; seen.add(d.getTime()); check = d; }
    } else break;
  }

  document.getElementById('ana-sessions').textContent = total_sessions;
  document.getElementById('ana-time').textContent = fmtShort(total_time);
  document.getElementById('ana-streak').textContent = streak + ' days';
  document.getElementById('ana-avg').textContent = fmtShort(avg_duration);

  // chart
  const dayMap = {};
  for (const c of filtered) { const d = c.completed_at.split('T')[0]; dayMap[d] = (dayMap[d]||0) + (c.total_duration/60); }
  ...
  for (const c of filtered) {
    ...
    bd[c.routine_name].count++; bd[c.routine_name].total_time += c.total_duration;
  }
  const bdList = Object.entries(bd).map(([k,v])=>({name:k,...v})).sort((a,b)=>b.count-a.count);
  document.getElementById('breakdown-list').innerHTML = bdList.map(b=>`
...
function saveStepModal() {
  ...
  const item = { id: uuid(), type: 'step', name, duration: dur, color };
...
function saveGroupModal() {
  ...
  document.querySelectorAll('#group-substeps .item-row').forEach(row => {
    ...
    if (n && d > 0) subSteps.push({ id: uuid(), name: n, duration: d, color: 'blue' });
  });
  ...
  const item = { id: uuid(), type: 'group', name, duration: total, color, steps: subSteps, loop_count: loops };
  if (idx !== undefined) editorItems[+idx] = item; else editorItems.push(item);
...
window.addEventListener('DOMContentLoaded', async () => {
  if (!localStorage.getItem('flowpace_device_id')) {
    localStorage.setItem('flowpace_device_id', uuid());
  }

  // load sound packs
  try { window.soundPacks = await api('GET', '/api/flowpace/soundpacks'); } catch { window.soundPacks = []; }
  ...
  if (!state.routines.length) {
    try {
      const seed = await api('POST', '/api/flowpace/seed');
      if (seed.seeded) {
        await syncFromServer();
      }
    } catch {
      // offline, create minimal sample
    }
    if (!state.routines.length) {
      // create a default routine locally
      ...
      await db.saveRoutine(defaultRoutine);
      state.routines.push(defaultRoutine);
    }
  }
  ...
  window.addEventListener('online', () => { state.isOnline = true; showToast('Back online'); syncToServer(); });
  window.addEventListener('offline', () => { state.isOnline = false; showToast('Offline mode'); });
