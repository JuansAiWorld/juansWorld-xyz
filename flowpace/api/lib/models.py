...
class RoutineItem(BaseModel):
    ...
    def from_step(cls, step: Step) -> RoutineItem:
        return cls(id=step.id, type="step", name=step.name, duration=step.duration, color=step.color)

    @classmethod
    def from_group(cls, group: Group) -> RoutineItem:
        return cls(
            id=group.id, type="group", name=group.name, duration=group.total_duration,
            color=group.color, steps=group.steps, loop_count=group.loop_count,
        )
