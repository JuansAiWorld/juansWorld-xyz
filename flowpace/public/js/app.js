...
async function loadData() {
  ...
  if (state.isOnline) {
    try {
      ...
      const serverData = await api('GET', '/api/sync');
...
async function duplicateRoutine(id) {
  ...
  for (const item of dup.steps) {
    ...
    if (item.steps) {
      for (const s of item.steps) s.id = crypto.randomUUID();
    }
  }
...
async function saveRoutine() {
  ...
  const id = state.editingRoutine ? state.editingRoutine.id : crypto.randomUUID();
...
window.addEventListener('DOMContentLoaded', async () => {
  if (!localStorage.getItem('flowpace_device_id')) {
    localStorage.setItem('flowpace_device_id', crypto.randomUUID());
  }
  ...
  if (!state.routines.length) {
    ...
    if (!state.routines.length) {
      ...
      const defaultRoutine = {
        ...
        name: "Quick HIIT",
        steps: [
          { id: crypto.randomUUID(), type: "step", name: "Jumping Jacks", duration: 30, color: "red" },
          { id: crypto.randomUUID(), type: "step", name: "Rest", duration: 15, color: "green" },
          { id: crypto.randomUUID(), type: "step", name: "Burpees", duration: 30, color: "orange" },
          { id: crypto.randomUUID(), type: "step", name: "Rest", duration: 15, color: "green" },
        ]
      };
